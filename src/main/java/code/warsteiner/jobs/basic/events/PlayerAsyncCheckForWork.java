	...
	public void onEvent(CreatureSpawnEvent event) {
		if (event.getSpawnReason().equals(org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason.SPAWNER)) {
			event.getEntity().setMetadata("spawned-by-spawner",
					new FixedMetadataValue(plugin, "spawned-by-spawner"));
		}
	}
								...
								if (chance2 < chance) {
									...
									if(real.getRewardMessages() != null) {
										...
										if(rw.containsKey("BOSSBAR")) {
											...
											String bm = plugin.getBasicPluginManager().toHex(rw.get("BOSSBAR").replaceAll("<block>", rld.getDisplay()).replaceAll("<job>", real.getDisplay()).replaceAll("<money>", ""+reward));
											...
											double use = BossBarHandler.calc(stats.getExp(),
													plugin.getLevelAPI().isMaxLevel(jbb, real.getID()), stats.getNeed());
										...
										if(rw.containsKey("ACTIONBAR")) {
											player.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(plugin.getBasicPluginManager().toHex(rw.get("ACTIONBAR").replaceAll("<block>", rld.getDisplay()).replaceAll("<job>", real.getDisplay()).replaceAll("<money>", ""+reward))));

										}
										if(rw.containsKey("MESSAGE")) {
											player.sendMessage(plugin.getBasicPluginManager().toHex(rw.get("MESSAGE").replaceAll("<block>", rld.getDisplay()).replaceAll("<job>", real.getDisplay()).replaceAll("<money>", ""+reward)));
										}
									}
									...
									new BukkitRunnable() {
										...
										@Override
										public void run() {
											PlayerFinishJobEvent event = new PlayerFinishJobEvent(ID, action, id, real.getID(), real);
