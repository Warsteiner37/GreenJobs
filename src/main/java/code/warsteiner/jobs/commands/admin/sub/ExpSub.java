...
public class ExpSub extends AdminSubCommand {
			...
			if (isDouble(value)) {
				...
				if (plugin.getJobAPI().existJob(job)) {
					...
					if (jb.getOwnedJobs().contains(rl.getID())) {
						...
						jb.getJobStats().get(rl.getID()).setExp(Integer.valueOf(value));

						sender.sendMessage(m.getPrefix(sender) + " §7Set §c" + player + "'s §7Exp in Job §a" + job + " §7to §6"
								+ value + ".");
						return;
					} else {
			...
			if (isDouble(value)) {
				...
				if (plugin.getJobAPI().existJob(job)) {
					...
					if (jb.getOwnedJobs().contains(rl.getID())) {
						...
						double old = jb.getJobStats().get(rl.getID()).getExp();
						
						Double nw = Double.valueOf(value);
						
						jb.getJobStats().get(rl.getID()).setExp(old+nw);

						sender.sendMessage(m.getPrefix(sender) +  " §7Added "+value+" Exp to the Job "+job+" of §c" + player + "!");
						return;
					} else {
						sender.sendMessage(m.getPrefix(sender) + " §7Error! Player does not own that Job!");
						return;
			...
			if (isDouble(value)) {
				...
				if (plugin.getJobAPI().existJob(job)) {
					...
					if (jb.getOwnedJobs().contains(rl.getID())) {
						...
						sender.sendMessage(m.getPrefix(sender) + " §7Removed "+value+" Exp from the Job "+job+" of §c" + player + "!");
						return;
					} else {
						sender.sendMessage(m.getPrefix(sender) + " §7Error! Player does not own that Job!");
						return;
	...
	public String getUsage() {
		return "§7/Jobsadmin §aexp §7<set,add,remove> <name> <job> <amount>";
	}

	@Override
	public String getPermission() {
		return "greenjobs.admin.exp";
	}
	...
	public String getArgsLayout() { 
		return "exp <set,add,remove> <name> <job> <amount>";
	}
